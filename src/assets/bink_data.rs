// Ported from FFmpeg's libavcodec/binkdata.h.
// Copyright (c) 2009 Konstantin Shishkov
// Licensed LGPL-2.1-or-later. See LICENSES/LGPL-2.1-or-later.txt in the repo root.

//! Lookup tables for the Bink 1 video decoder.
//!
//! All tables are reproduced verbatim from FFmpeg's `libavcodec/binkdata.h`
//! at commit `9acd820732f0bf738bd743bbde6a5c3eadc216c2`. They are
//! reverse-engineered constants from the RAD BINKW32.DLL, originally
//! captured by the FFmpeg authors.
...
/// Huffman code bit patterns for the 16 fixed Bink trees.
...
/// Huffman code bit lengths for the 16 fixed Bink trees.
...
/// Block scan patterns for SCALED / PATTERN / RAW blocks — each pattern is a
/// permutation of 0..63 mapping decode-order slots to 8×8 pixel positions.
...
/// Intra block quantization matrices (16 quality levels × 64 coefficients).
...
/// Inter block quantization matrices (16 quality levels × 64 coefficients).
...
/// Number of bits used to store the first DC value in a DC bundle.
/// From `libavcodec/bink.c:501`.
pub const DC_START_BITS: u32 = 11;

/// RLE expansion lengths for the `read_block_types` escape values 12..15.
/// From `libavcodec/bink.c:389`.
...
mod tests {
    ...
    #[test]
    fn scan_is_permutation_of_0_to_63() {
        let mut seen = [false; 64];
        for &v in &BINK_SCAN {
            assert!(v < 64, "BINK_SCAN value {} out of range", v);
            assert!(!seen[v as usize], "BINK_SCAN duplicate {}", v);
            seen[v as usize] = true;
        }
        assert!(seen.iter().all(|&s| s), "BINK_SCAN missing values");
    }

    #[test]
    fn tree_bits_and_lens_have_16_by_16_shape() {
    ...
    #[test]
    fn every_tree_covers_its_code_space_exactly() {
        // Kraft inequality: sum of 2^(maxlen - len_i) over all symbols == 2^maxlen.
        for t in 0..16 {
            let max_len = *BINK_TREE_LENS[t].iter().max().unwrap() as u32;
            let total: u32 = BINK_TREE_LENS[t]
                .iter()
                .map(|&l| if l == 0 { 0 } else { 1 << (max_len - l as u32) })
                .sum();
            assert_eq!(total, 1 << max_len, "tree {} does not tile", t);
        }
    }

    #[test]
    fn patterns_are_permutations_of_0_to_63() {
        for p in 0..16 {
            let mut seen = [false; 64];
            for &v in &BINK_PATTERNS[p] {
                assert!(v < 64);
                seen[v as usize] = true;
            }
            assert!(seen.iter().all(|&s| s), "pattern {} incomplete", p);
        }
    }

    #[test]
    fn quant_matrices_are_nonzero() {
        // Sanity only — if we pasted a zero row, everything IDCT-related breaks.
        for q in 0..16 {
            let intra_sum: i64 = BINK_INTRA_QUANT[q].iter().map(|&v| v as i64).sum();
            let inter_sum: i64 = BINK_INTER_QUANT[q].iter().map(|&v| v as i64).sum();
            assert!(intra_sum > 0, "intra quant row {} is empty", q);
            assert!(inter_sum > 0, "inter quant row {} is empty", q);
        }
    }
}
